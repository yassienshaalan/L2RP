      ...
      for line in fp:
           ...
           try:
              with open(productFileName, 'r') as filep:
                ...
                for dayweight in dayweights:
                    part1 = (dayweight/sumdayweights)*100
                    part2 = credweights[index]*10
                    print(part1)
                    print(part2)
                    part3 = part1 + part2
                    print(part3)
            ...
            for reviewline in productfp:
                ...
                try:
                    ...
                    if review[4] != "":
                        ...
                        reviewVotes[0] = int(reviewVotes[0])
                    if review[3] != "" and reviewVotes[1]!="":
                        reviewVotes[1] = int(reviewVotes[1]) + int(review[3])
                    else:
                        if reviewVotes[1] != "":
                            reviewVotes[1] = int(reviewVotes[1])
...
def countNumberOFGoodVotesForUsersPerCategory():
    ...
    return
def getProductReviewTimeSpanAllCategories(directory):
    print("Started")
    start = datetime.now()
    print(start)
    for filename in os.listdir (directory):
        ...
        categoryName = category.split(".txt")
        categoryName = categoryName[0]
        dateSectors = [0,0,0,0,0,0,0,0]
        getProductReviewTimeSpan(path1,categoryName,dateSectors)


    print("Done")
    return

def getProductReviewTimeSpan(path1,category,dateSectors):
    line = ""
    product = ""
    print("Started getProductReviewTimeSpan")
    print("Considering "+category)
    userHelpful = dict()
    with open(path1, 'r') as fp:
      for line in fp:
        ...
        minDate = datetime(2050, 12, 31)
        maxDate = datetime(1950, 1, 1)
        for date in productDates:
            if minDate > date:
                minDate = date
            if maxDate<date:
                maxDate = date
        dateDifference = (maxDate-minDate).days
        newDateDiff = dateDifference/8
        numDivisions = 7
        numMonths = newDateDiff/30
        thresholdDates = []
        nextDate = minDate
        thresholdDates.append(minDate)
        for i in range(numDivisions):
            newMon = nextDate.month + numMonths
            newYear = nextDate.year
            if newMon > 12:
                numYears = newMon/12
                newMon = newMon - numYears*12
                if newMon == 0:
                    newMon = 1
                newYear = newYear + numYears

            nextDate = datetime(newYear,newMon,1)
            '''if nextDate.month == 12:
                nextDate = datetime(nextDate.year+1,1,1)
            else:
                nextDate = datetime(nextDate.year,nextDate.month+1,1)'''
            thresholdDates.append(nextDate)
        thresholdDates.append(maxDate)
        totalRevs = 0

        for i in range(len(thresholdDates)-1):
            numInRange = 0
            first = thresholdDates[i]
            second = thresholdDates[i+1]
            for date in productDates:
                if date >= first and date <second:
                    numInRange = numInRange + 1
            sector = dateSectors[i]
            sector = sector + numInRange
            dateSectors[i] = sector
            totalRevs = totalRevs + numInRange
        #print("Total Revs " + str(totalRevs))
    filehandle = open("/research/remote/petabyte/users/yassien/Unique_Products_Stanford_three/ProductReviewDateDistribution/"+category+".txt",'a')
    index = 0
    finalStr = ""
    for sector in dateSectors:
        filehandle.write(str(index))
        filehandle.write("\t")
        filehandle.write(str(sector))
        finalStr = finalStr + "\t" + str(sector)
        filehandle.write("\n")
        index = index + 1
    print(finalStr)
    return
...
#computeTrueRateForAllCategoreis("/research/remote/petabyte/users/yassien/Unique_Products_Stanford_three/categories/",13,"/research/remote/petabyte/users/yassien/Unique_Products_Stanford_three/categories_weighted_date_vote_expo_test/")

#sortRatedCategories("/research/remote/petabyte/users/yassien/Unique_Products_Stanford_three/categories_weighted_date_vote_expo_test/","/research/remote/petabyte/users/yassien/Unique_Products_Stanford_three/categories_sorted_weighted_date_vote_expo_test/")

#measureDistanceBetweenSalesRankandMajorityVote("/research/remote/petabyte/users/yassien/Unique_Products_Stanford_three/categories_sorted_sales_rank/","/research/remote/petabyte/users/yassien/Unique_Products_Stanford_three/categories_sorted_weighted_date_vote_expo_test/","/research/remote/petabyte/users/yassien/Unique_Products_Stanford_three/categories_differences_wegighted_date_vote_local_expo_test_R/")
...
#getProductReviewTimeSpan("/research/remote/petabyte/users/yassien/Unique_Products_Stanford_three/Product_Reviews/B000WOV83M.txt","B000WOV83M",dateSector)
getProductReviewTimeSpanAllCategories("/research/remote/petabyte/users/yassien/Unique_Products_Stanford_three/categories/")